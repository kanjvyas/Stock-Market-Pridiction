...
if data.empty:
    ...
    scaler = MinMaxScaler()
    X = data[['Open', 'High', 'Low', 'Close', '7_day_MA', '14_day_MA', 'Daily_Return']]
    ...
    # Model Training
    model = XGBRegressor(n_estimators=100, max_depth=5, learning_rate=0.1, subsample=0.8)
    model.fit(X_train, y_train)
    y_pred = model.predict(X_test)
    ...
    # Future Forecast
    future_days = 10
    last_data = X_scaled[-5:]  # Use the last 5 rows for prediction
    future_predictions = []

    for _ in range(future_days):
        pred_price = model.predict(last_data[-1].reshape(1, -1))
        future_predictions.append(pred_price[0])
        new_row = np.append(last_data[-1][1:], pred_price)
    ...
    st.subheader("🔮 Forecasted Prices (Next 10 Business Days)")
    future_dates = pd.date_range(start=pd.Timestamp.now(), periods=future_days, freq='B')
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=future_dates, y=future_predictions, mode='lines+markers', name="Forecasted Prices", line=dict(color='#00ff7f')))
    st.plotly_chart(fig2, use_container_width=True)

    st.subheader("📋 Forecasted Prices Table")
    forecast_df = pd.DataFrame({"Date": future_dates.date, "Forecasted Price": future_predictions})
    st.dataframe(
        forecast_df.style.set_properties(**{'background-color': '#1e1e1e', 'color': '#ffffff', 'border': '1px solid #444'})
    )
...
# Footer
st.markdown("---")
st.write("© 2024 Stock Market Prediction App. All rights reserved.")
